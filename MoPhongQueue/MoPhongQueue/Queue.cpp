...
int main()
{
	...
	initgraph();
	int* a = new int[10]; // luu cac phan tu chua duoc them vao hang doi
	int* b = new int[10]; // luu cac phan tu co trong hang doi
	int* c = new int[10]; // luu cac phan tu da duoc xoa khoi hang doi

	int n;
	cout << "Nhap so luong phan tu: ";
	cin >> n;

	//khoi tao cac bien duoc su dung cho viec hien thi hinh anh va chuyen dong
	...
	//dem: dem cac phan tu chua duoc them vao hang doi
	//dem1: dem cac phan tu dang co trong hang doi
	//dem2: dem cac phan tu da xoa ra khoi hang doi
	...
	while (true)
	{
		...
		if (select == 1)
		{
			if (dem == 0) // tat ca phan tu da duoc them vao hang doi
			{
				setcolor(9);
			...
			else if (dem1 == 7) drawNoti(1250, y - 200, "HANG DOI DA DAY");
